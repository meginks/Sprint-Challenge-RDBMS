...
RDBMS is an acronym for relational database management system. It is the model that SQL (structured query language) is based on. Basically, SQL is an example of an RDBMS, but an RDBMS doesn't necessarily have to be SQL. 

1. Why do tables need a `primary key`?

A primary key is what ensures that your records in your table are identifiable and can protect against situations where there may be records that look duplicate but are not. Primary keys are necessary in the 1NF normal form. 
